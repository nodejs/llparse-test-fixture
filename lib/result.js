...
class FixtureResult {
    ...
    async check(input, expected, options = {}) {
        ...
        const rawLength = buffer_1.Buffer.byteLength(input);
        ...
        const results = await Promise.all(ranges.map((range) => this.spawn(range, input)));
    ...
    async spawnSingle(executable, range, input) {
        ...
        const proc = child_process_1.spawn(executable, [
            `${range.from}:${range.to}`,
            input,
        ], {
        ...
        const out = buffer_1.Buffer.concat(stdout).toString()
            .split(/===== SCAN \d+ START =====\n/g).slice(1);
        return {
            name,
