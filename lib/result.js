...
class FixtureResult {
    ...
    async spawnSingle(executable, range, input) {
        ...
        const proc = (0, child_process_1.spawn)(executable, [
            `${range.from}:${range.to}`,
            input,
        ], {
        ...
        await onEnd;
        const stdoutText = buffer_1.Buffer.concat(stdout).toString();
        const stderrText = buffer_1.Buffer.concat(stderr).toString();
