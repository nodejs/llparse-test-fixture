...
import { ERROR_PAUSE } from './fixture';

export default (binding: any, inBench: boolean) => {
  let globalOff = 0;
  binding.setGlobalOff = (value: number) => globalOff = value;

  const translate = (off: number) => off + globalOff;

  const nop = () => 0;
  binding.llparse__print = inBench ? nop :
    (off: number, fmt: string, ...params: any[]) => {
      const parts = [
  ...
  binding.llparse__print_span = inBench ? nop :
    (name: string, buf: Buffer, off: number, end: number) => {
      let last = off;

      const printRegular = (current: number) => {
        if (last !== current) {
  ...
  binding.debug = inBench ? nop :
    (_: any, buf: Buffer, off: number, msg: string) => {
      if (off === buf.length) {
        // tslint:disable-next-line:no-console
